...
(function($) {
	$.fn.pignoseParallaxSlider = function(options) {
		var pignoseParallaxSlider = {
			init : function(options) {
				...
				return this.each(function() {
					...
					$view.addClass('pignose-parallaxslider');
					...
					if(typeof _this.settings.play !== 'undefined' &&
					   _this.settings.play !== null
						...
						$target.find(_this.settings.play).unbind('click.slideControlHandler').bind('click.slideControlHandler', function(event) {
							...
							t = setInterval(function() {pignoseParallaxSlider.move.apply(_this, new Array(_this.settings.direction))}, _this.settings.interval);
					...
					if(typeof _this.settings.prev !== 'undefined' &&
					   _this.settings.play !== null
						...
						$target.find(_this.settings.prev).unbind('click.slideControlHandler').bind('click.slideControlHandler', function(event) {
							pignoseParallaxSlider.move.apply(_this, new Array(((_this.settings.direction == 'left')? 'right':'left'), 1, true));
							event.preventDefault();
						});
					}
					...
					if(typeof _this.settings.next !== 'undefined' &&
					   _this.settings.play !== null
						...
						$target.find(_this.settings.next).unbind('click.slideControlHandler').bind('click.slideControlHandler', function(event) {
							pignoseParallaxSlider.move.apply(_this, new Array(_this.settings.direction, 1, true));
							event.preventDefault();
						});
					}
							   ...
							   _this.settings.pagination === true
								...
								if($pagination.length > 0 && !$pagination.hasClass('completedPagination')) {
									...
									$page.bind('click.slideControlHandler', function(event) {
										...
										pignoseParallaxSlider.move.apply(_this, new Array(direction, offset, true));
										event.preventDefault();
									});
		...
		if(pignoseParallaxSlider[options]) {
			return pignoseParallaxSlider[options].apply(this, Array.prototype.slice.call(arguments, 1));
		} else if (typeof options === 'object' || !options) {
			return pignoseParallaxSlider.init.apply(this, arguments);
		} else {
